 ...
 $services = explode(",", get_option('custom_services_list')); 
						...
						<ul id="services">
							<?php foreach($services as $k=>$v):
									...
									$k=sanitize_data($k);
									$v=sanitize_data($v);
								if(get_post_meta( $id,$v,true)=='has') {$css='foundicon-checkmark';} else { $css='foundicon-remove';}								
								$v=explode('|',$v);
							...
							?>                        
							<li>
	                            <i class="<?php echo $css; ?>">
	                            <?php echo unsanitize_data($v); ?>
	                            </i>
							</li>
							<?php	endforeach; ?>		
					     </ul>
...
{
if($limit=='') {$limit=-1;} else {$limit=$limit;}
...
if ($attachments) {
	echo '<ul class="block-grid three-up" data-clearing>';
	foreach ($attachments as $attachment) {
		//echo apply_filters('the_title', $attachment->post_title);
		$img=wp_get_attachment_url($attachment->ID, 'large', false);
		//echo '<a href="'.$img[0].'" rel="lightbox">';
				
		
		
		echo '<li id="file-'.$attachment->ID.'">

<input type="hidden" name="nonce" id="nonce" value="'.wp_create_nonce( 'delete_attachment').'" />
		
		<a href="'.$img.'">'.wp_get_attachment_image($attachment->ID, $size, false, false).'</a>
				<a class="remImage" name="'.$attachment->ID.'" href="#">cancella</a>  <input type="hidden" id="att_remove" name="att_remove[]" value="'.$attachment->ID.'" /></li>';
		//echo '</a>';
	}
	echo '</ul>';	
} 
